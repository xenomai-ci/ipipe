...
#define WD_MPC5200_COUNT	0xffff /* default count value for gpt0 */
...
int wd_mpc5200_init(unsigned long *tpp)
{
	...
	/* check if we have gpt0 */
	if (mpc52xx_gpt0 == NULL) {
		printk("%s: couldn't find gpt0\n", __FUNCTION__);
		return -ENXIO;
	}
	...
	/* disable watchdog */
	mpc52xx_gpt0->mode = 0x00000000;

	/* configure watchdog */
	mpc52xx_gpt0->mode |= 0x00000004;
	mpc52xx_gpt0->mode |= 0x00001000;

	/* set watchdog timeout */
	mpc52xx_gpt0->count = (prescale << 16) | WD_MPC5200_COUNT;

	/* enable watchdog */
	mpc52xx_gpt0->mode |= 0x00008000;
